// Korean (ko) strings; mirrors the keys in en.ts.

const ko: Record<string, string> = {
  // header
  lang_label: "언어",
  settings_title: "설정",
  settings_lang: "언어",
  setting_units: "크기 표기",
  units_human: "KiB / MiB",
  units_bytes: "바이트",
  units_hex: "16진수",
  help_units: "페이지의 모든 크기를 어떻게 표기할지 정합니다. 반올림은 읽기 편하지만 서로 다른 두 값을 같아 보이게 할 수 있습니다. 파티션과 그 안의 이미지는 수 킬로바이트 차이가 나도 같은 값으로 반올림되기 때문입니다. 16진수는 플래시 주소 표기와 맞춰집니다.",
  btn_close: "닫기",
  title_settings: "설정",
  title_home: "처음으로 돌아가기",
  title_close: "닫기",
  title_expand: "전체 화면으로 확대",
  title_prev: "이전 빌드",
  title_next: "다음 빌드",
  title_snapshot: "읽을 스냅샷",
  ctx_hook: "훅",
  ctx_inferred: "추론",
  ctx_artifact: "산출물",
  kernel_prefix: "linux",
  warning_one: "경고 1개",
  warning_many: "경고 {n}개",
  add_report: "보고서 추가",
  loading: "불러오는 중",
  loading_reports: "보고서 로드 중… {pct}",

  // fleet overview
  fleet_title: "플릿",
  fleet_filter: "빌드 필터",
  fleet_builds: "빌드 {n}개",
  fleet_missing: "{name}은(는) 이 릴리스에서 빌드되지 않았습니다.",
  view_table: "표",
  view_map: "맵",
  view_numbers: "수치",
  th_build: "빌드",
  th_branch: "브랜치",
  th_flash: "플래시",
  th_rootfs: "루트FS",
  fleet_entry_title: "Thingino 릴리스",
  fleet_entry_latest: "최신 항목 열기",
  fleet_entry_pick: "릴리스 선택",
  fleet_entry_sub: "릴리스의 모든 카메라에 대한 파티션, 패키지, 커널 상세 분석.",
  back_to_fleet: "모든 장치",

  // tabs
  tab_flash: "플래시",
  tab_env: "환경 변수",
  tab_dtb: "디바이스 트리",
  kconfig_title: "커널 설정",
  kconfig_counts: "{total}개 중 {set}개 설정됨",
  filter_options: "옵션 필터",
  kconfig_show_unset: "미설정 표시",
  th_option: "옵션",
  tab_packages: "패키지",
  tab_files: "파일",
  tab_kernel: "커널",
  tab_time: "빌드",
  tab_drift: "증감",

  // flash tab
  flash_map: "플래시 맵",
  device: "장치",
  diemap_note: "폭은 용량에 비례하며, 작은 파티션은 가독성을 위해 확대했습니다",
  no_layout:
    "이 빌드의 파티션 레이아웃을 찾을 수 없습니다(mtdparts 소스나 파티션 테이블 없음). 아래 산출물 크기는 여전히 정확합니다.",
  th_partition: "파티션",
  th_range: "범위",
  th_size: "크기",
  th_image: "이미지",
  th_content: "내용",
  th_used: "사용량",
  th_free: "여유",
  th_fill: "점유율",
  th_check: "검증",
  help_col_size: "파티션 테이블이 이 파티션에 배정한 플래시 용량입니다.",
  help_col_content: "그 안에 기록된 이미지의 크기입니다. 파티션 크기와는 다르며, 반올림하면 같아 보이는 일이 잦습니다. 설정에서 정확한 바이트를 켜면 차이가 보입니다.",
  help_col_used: "그 이미지에서 실제로 사용된 양입니다. 파일 시스템은 보통 파티션을 채우도록 만들어지므로 이미지가 내용물보다 훨씬 클 수 있습니다.",
  help_col_free: "파티션 크기에서 사용량을 뺀 값입니다. 이 빌드에 남은 여유입니다.",
  verified: "검증됨",
  mismatch: "불일치",
  verified_title: "내용이 플래시 이미지와 일치함을 확인했습니다",
  mismatch_title: "내용이 플래시 이미지와 일치하지 않습니다",
  read_only_short: "ro",
  spans: "(중복)",
  env_title: "U-Boot 환경 변수",
  n_vars: "변수 {n}개",
  th_variable: "변수",
  th_value: "값",
  filter_vars: "변수 필터",
  vars_matching: "{n}개 일치",
  vars_capped: "앞부분의 변수만 표시합니다.",
  filter_nodes: "노드와 속성 필터",
  n_properties: "속성 {n}개",
  dtb_capped: "앞부분의 노드만 표시합니다.",
  device_trees: "디바이스 트리",
  n_trees: "{n}개 발견",
  th_board: "보드",
  th_found_in: "발견 위치",
  th_nodes: "노드",
  th_bootargs: "커널 명령줄",
  dtb_in_image: "로더",
  dtb_in_payload: "U-Boot",
  help_dtb_image: "배포되는 바이너리 안에서 찾은 트리입니다. 부트로더의 경우 로더 자신의 트리로, DRAM을 올리고 플래시를 읽는 데 필요한 것만 기술합니다.",
  help_dtb_payload: "로더가 푸는 압축 페이로드 안에서 찾은 트리입니다. 이쪽이 실제 보드 트리로, 해당 카메라에 설정된 GPIO와 주변장치가 들어 있습니다.",
  device_trees_note:
    "로우 플래시에서 부팅하는 보드는 보통 이를 파일로 제공하지 않습니다. 부트로더는 자신의 바이너리에 트리를 덧붙이고, 커널은 자체 압축 안쪽에 하나를 품고 있습니다.",
  ubi_volumes: "UBI 볼륨",
  th_volume: "볼륨",
  th_type: "종류",
  th_blocks: "블록",
  th_reserved: "예약",
  th_payload: "데이터",
  th_on_flash: "플래시 사용",
  volume_unwritten: "기록 없음",
  volume_holes: "블록 누락",
  ubi_volumes_note:
    "예약은 볼륨 테이블이 확보해 둔 크기입니다. 아무것도 기록되지 않은 볼륨은 아직 플래시를 차지하지 않으며, autoresize 볼륨은 커널이 처음 연결할 때 칩 전체를 채울 만큼 늘어납니다.",
  images_dir: "images/",
  n_files: "{n}개 파일",
  th_file: "파일",
  th_format: "형식",
  th_introspection: "내부 정보",
  warnings_title: "경고",
  tt_partition: "파티션",
  tt_used: "사용량",
  tt_free: "여유",
  tt_image: "이미지",

  // packages tab
  stat_rootfs_uncompressed: "rootfs 비압축",
  stat_compressed: "압축 후({algo})",
  stat_packages: "패키지",
  stat_files_in_rootfs: "rootfs 내 파일",
  filter_packages: "패키지 필터",
  sort_bytes: "비압축 크기순",
  sort_approx: "압축 후 추정순",
  sort_files: "파일 수순",
  sort_name: "이름순",
  cat_apps: "애플리케이션",
  cat_kernel: "커널 + 모듈",
  cat_libraries: "라이브러리",
  cat_firmware: "펌웨어 / blob",
  cat_base: "기본 시스템",
  cat_overlay: "overlay / 빌드 후",
  toggle_category: "{name} 표시 전환",
  pkg_table_title: "패키지",
  pkg_table_sub: "{total}개 중 {n}개",
  pkg_map_title: "rootfs 점유 비율",
  th_package: "패키지",
  th_bytes: "바이트 수",
  th_flash_approx: "~플래시",
  th_files: "파일 수",
  th_share: "비중",
  overlay_post_build: "(overlay / 빌드 후)",
  of_rootfs: "(rootfs 대비)",
  compressed_approx: "압축 후 약 {size}",
  not_shipped_title: "설치되었지만 미포함",
  not_shipped_sub: "이미지 생성 전 {n}개 파일 제거 · 설치 시 {size}",
  th_path: "경로",
  th_install_size: "설치 크기",
  show_all: "전체 {n}개 표시",
  show_top: "상위만 표시",

  // files tab
  files_title: "파일",
  files_source_rootfs: "rootfs(패키지별 분류)",
  files_total: "파일 수",
  files_bytes: "비압축",
  filter_paths: "경로 필터",
  files_matching: "{n}개 일치",
  files_capped: "목록이 잘렸습니다. 일부 파일은 표시되지 않습니다.",
  no_files: "이 보고서에는 파일 목록이 없습니다.",
  no_matches: "이 필터와 일치하는 항목이 없습니다.",

  // modules tab
  stat_modules: "모듈",
  stat_total_size: "전체 크기",
  stat_autoloaded: "자동 로드(/etc/modules)",
  stat_kernel: "커널",
  stat_builtin: "내장(커널 이미지 내)",
  stat_config: "커널 설정",
  on_demand_only: "온디맨드만(자동 로드 아님)",
  th_module: "모듈",
  filter_modules: "모듈 필터",
  th_load: "로드",
  load_auto: "자동",
  load_on_demand: "온디맨드",
  modules_title: "로드 가능 모듈",
  modules_sub: "{n}개 · {size}",
  no_modules: "이 rootfs에는 커널 모듈이 없습니다.",

  // build time tab
  stat_active_build_time: "실제 빌드 시간",
  stat_packages_timed: "측정된 패키지",
  stat_finished: "완료 시각",
  no_build_log: "이 트리에는 build-time.log가 없습니다.",
  timings_title: "패키지 빌드 시간",
  timings_sub: "{n}개 패키지 · 작업 시간 {total}",
  help_package_work: "각 패키지의 빌드 시간을 합한 값입니다. 병렬 빌드에서는 경과 시간을 넘어섭니다. 동시에 진행된 작업이 여기서는 패키지마다, 실제 빌드 시간에서는 한 번만 계산되기 때문입니다.",
  help_active_build_time: "빌드가 컴파일한 시간입니다. 계측된 단계들의 합집합이라 병렬로 진행된 작업은 한 번만 계산됩니다. 체크아웃과 패키징을 포함한 전체 작업보다 짧고, 패키지 시간의 총합보다도 짧습니다.",
  stat_profile: "프로파일",
  stat_build_config: "빌드 설정",
  profile_title: "카메라 프로파일 (defconfig)",
  build_config_title: "빌드 설정",
  n_options: "{n}개 항목",
  help_profile: "이 카메라를 빌드한 defconfig를 작성된 그대로 보여줍니다. SoC, 센서, 플래시 용량, 선택한 기능들입니다. 수십 줄에 불과하지만 Kconfig는 여기서 수백 개 항목을 도출합니다.",
  help_build_config: "Buildroot가 실제로 빌드에 사용한 모든 항목으로, 프로파일을 확장한 결과입니다. 크기나 동작을 설명하는 것은 이쪽이고, 카메라를 식별하는 것은 프로파일입니다.",
  stat_device_config: "장치 설정",
  device_config_title: "장치 설정",
  help_device_config: "rootfs에서 가져온 설정 파일을 장치가 읽는 그대로 보여줍니다. 빌드 시스템에는 이 정보가 없습니다. 어떤 GPIO가 IR 컷 필터를 구동하는지, 팬 모터가 몇 스텝인지는 여기에서만 정해집니다.",

  // drift tab
  baseline: "기준",
  compared_against: "현재 빌드({name})와 비교",
  loading_baseline: "기준을 불러오는 중",
  compared_against_release: "{tag}의 {name}과(와) 비교",
  not_in_release: "{name}은(는) {tag}에서 빌드되지 않았습니다",
  help_baseline_release: "같은 카메라를 비교할 이전 릴리스입니다. 아래 표는 해당 스냅샷과 현재 보고 있는 스냅샷 사이의 변경 사항을 보여줍니다.",
  help_baseline_build: "비교할 다른 빌드입니다.",
  stat_rootfs_compressed: "rootfs 압축 후",
  stat_packages_changed: "패키지 변경",
  stat_modules_changed: "모듈 변경",
  stat_partitions_changed: "파티션 변경",
  stat_config_changed: "커널 설정 변경",
  stat_env_changed: "환경 변수 변경",
  drift_config: "커널 설정",
  drift_env: "U-Boot 환경 변수",
  stat_build_config_changed: "빌드 설정 변경",
  drift_build_config: "빌드 설정",
  stat_device_config_changed: "장치 설정 변경",
  drift_device_config: "장치 설정",
  help_build_config_changed: "두 빌드에서 다르게 설정된 Buildroot 옵션입니다. 패키지가 생기거나 크기가 달라지는 일은 대개 여기서 시작됩니다.",
  help_device_config_changed: "rootfs에서 가져온 설정 파일 중 값이 다른 항목을, 파일과 그 안의 설정 단위로 보여줍니다.",
  config_incomparable: "두 빌드의 커널이 다릅니다({before}, {after}). 거의 모든 옵션이 달라 비교해도 의미가 없습니다.",
  help_config_changed: "두 빌드에서 다르게 설정된 커널 옵션입니다. 열면 항목과 값을 볼 수 있습니다.",
  help_env_changed: "서로 다른 U-Boot 변수입니다. 보드가 부팅할 때 쓰는 파티션 구성을 포함합니다.",
  range_from_to: "{from} → {to}",
  partitions_used_bytes: "파티션(사용 바이트)",
  th_baseline: "기준",
  th_current: "현재",
  th_delta: "증감",
  th_partition_size: "파티션 크기",
  th_name: "이름",
  chip_new: "신규",
  chip_alias: "별칭",
  help_alias_partition: "이 파티션은 다른 파티션들을 포괄하므로 해당 바이트는 이미 각 행에 계산되어 있습니다. 변화를 표시하면 이중 계산이 되므로 차이를 표시하지 않습니다.",
  chip_removed: "제거",
  n_changed: "{n}개 변경",
  no_differences: "두 빌드 간 차이가 없습니다.",
  drift_packages: "패키지",
  drift_images: "이미지",
  drift_modules: "커널 모듈",

  // drop zone
  drop_title: "펌웨어 이미지, 이미지 폴더 또는 보고서를 끌어다 놓으세요",
  drop_sub_html:
    "Buildroot로 빌드한 모든 장치를 지원합니다. 순수 <code>.bin</code>은 이미지 자체에서 파티션 맵을 추출하고, <code>buildscope-report.json</code>은 그대로 표시합니다. 모든 처리는 이 브라우저에서 이루어지며 업로드하지 않습니다.",
  drop_cli_hint:
    "Buildroot 출력 디렉터리를 분석하려면 빌드 트리가 필요하므로 이 작업은 로컬에서 실행합니다: <code>buildscope export output/my-build</code>.",
  choose_files: "파일 선택",
  choose_build_dir: "빌드 디렉터리 선택",
  working: "처리 중: {what}",
  stage_reading: "{name} 읽는 중",
  stage_scanning: "{name} 스캔 중",
  stage_analyzing_file: "{name} 분석 중",
  stage_reading_inputs: "빌드 입력을 읽는 중",
  stage_indexing_target: "target 트리 인덱싱 중",
  stage_reading_images: "이미지를 읽는 중",
  stage_analyzing: "분석 중",
  not_a_build_dir: "{name}: Buildroot 출력 디렉터리가 아닙니다",

  // footer
  scan_mode: "스캔 {mode}",
  report_by: "보고서 생성: {name} {version}",
  // help balloons
  help_ctx_hook: "Where the build's paths came from: Buildroot stated them, having run buildscope as a post-image script. Says nothing about who did the reading -- that is the scan mode in the footer.",
  help_ctx_inferred: "Where the build's paths came from: worked out from the tree layout, by finding target/, build/, images/ and .config where they usually sit. A hooked build has Buildroot state them instead; both produce the same report.",
  help_ctx_artifact: "There was no build tree. Everything here was read out of the firmware image itself, so packages, modules and build times are absent rather than empty.",
  help_scan_native: "Who read the tree: the command line, walking a real filesystem. It can see hardlinks, so a file linked twice is charged once.",
  help_scan_browser: "Who read the tree: a browser tab. The File API exposes no hardlinks, so a hardlinked file is counted once per path. Buildroot target trees rarely contain any.",
  title_help: "도움말",
  help_help: "Turns the hints on and off. With them on, hovering a control explains it.",
  help_settings: "Language, and whether these hints are shown.",
  setting_help_label: "도움말 표시: 컨트롤 위에 마우스를 올리면 설명이 나옵니다",
  help_open_latest: "Opens the newest published snapshot: every camera in that release, with its flash usage.",
  help_all_devices: "Back to the list of every build in this snapshot.",
  help_snapshot: "Which published release to read. It stays on the device you are reading, when that release has it, so you can follow one camera across releases. Switching reloads the page, so the address always says which one you are looking at.",
  help_stepper: "Step to the previous or next build, keeping the tab you are on. Useful for comparing the same view across devices.",
  help_fleet_view: "Table lists how full each build is. Map draws every flash layout to one scale, so a device partitioned differently stands out.",
  help_fleet_numbers: "Adds each partition's size and fill as aligned columns, for the segments too small to read as bars.",
  help_builtin: "Drivers compiled into the kernel rather than shipped as .ko files. They are part of the kernel image, so they have no size of their own.",
  help_kconfig: "The kernel's own .config, recovered from the image itself. The count is the options actually set.",
  help_show_unset: "Also list options explicitly disabled. Most of the file is these, so they are hidden by default.",
  help_treemap_expand: "Expand the treemap to the whole window, where far more of the packages are large enough to label. Escape closes it.",
};

export default ko;
